...
def step(context):
    assert context.saved_text, (
        'context.saved_text is %r!!' % (context.saved_text,)
    )
...
def step(context):
    assert context.saved_table, (
        'context.saved_table is %r!!' % (context.saved_table,)
    )
...
def step(context, tag):
    assert tag not in context.tags, (
        '%r IS present in %r!' % (tag, context.tags)
    )
