...
def step_i_run_command(context, command):
    ...
    if False and DEBUG:
        ...
        print("XXX run_command.outout {0}".format(
...
def step_command_output_should_contain_exactly_text(context, text):
    ...
    # REAL: textutil.assert_text_should_contain_exactly(actual_output,
    # expected_text)
...
def step_command_output_should_contain_not_exactly_with_multiline_text(
        context):
    assert context.text is not None, "ENSURE: multiline text is provided."
